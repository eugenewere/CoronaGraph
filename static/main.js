...
function getError(results) {
	console.log(results);
	setTimeout(function () {
		$('.covid_error').removeClass('d-none');
	}, 3000);
	$('#vmap1').vectorMap({
		map: 'world_en',
		...
		if (x['CountryCode'] === code.toUpperCase()) {
			...
			$('#body_modal').append(
				'<div class="col-sm-12 col-md-12 col-lg-5">' +
					'<h3 class="custom-title"><i class="fa fa-info-circle"></i> ' +
					x['Country'] +
					'</h3>' +
		...
		$('#myTable')
			...
			.append(
				'<tr >' +
					...
					'  id=' +
					x['CountryCode'] +
					" onclick='eachCountryStats(this)'  href='#'>" +
					x['Country'] +
					'</a>' +
					'</td>' +
...
function updateSideMapData(results) {
	...
	$('#side_map_data').append(
		"<li><span style='font-weight: bolder;'>NewConfirmed:</span>" +
			' ' +
			x['NewConfirmed'] +
			'</li>' +
			"<li><span style='font-weight: bolder;'>TotalConfirmed:</span>" +
			' ' +
			x['TotalConfirmed'] +
			'</li>' +
			"<li><span style='font-weight: bolder;'>NewDeaths:</span>" +
			' ' +
			x['NewDeaths'] +
			'</li>' +
			"<li><span style='font-weight: bolder;'>TotalDeaths:</span>" +
			' ' +
			x['TotalDeaths'] +
			'</li>' +
			"<li><span style='font-weight: bolder;'>NewRecovered:</span>" +
			' ' +
			x['NewRecovered'] +
			'</li>' +
			"<li><span style='font-weight: bolder;'>TotalRecovered:</span>" +
			' ' +
			x['TotalRecovered'] +
			'</li>'
	);
